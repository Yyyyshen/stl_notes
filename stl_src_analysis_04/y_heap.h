...
public:
	y_priority_queue() :c() {}
	explicit y_priority_queue(const Compare& x) :c(), comp(x) {}
	//任何一个构造函数都立刻在底层容器产生一个implicit representation heap
	template<class InputIterator>
	y_priority_queue(InputIterator first, InputIterator last, const Compare& x) : c(first, last), comp(x) { make_heap(c.begin(), c.end(), comp); }
	template<class InputIterator>
	y_priority_queue(InputIterator first, InputIterator last) : c(first, last) { make_heap(c.begin(), c.end(), comp); }
	//操作函数
